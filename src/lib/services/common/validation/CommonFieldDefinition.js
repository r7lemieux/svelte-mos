...
export const BaseFieldDefs = {
    Id: new FieldDefinition({
        ...
        // cellRenderer: params => '<a href="https://www.google.com" target="_blank" rel="noopener">'+ params.value+'</a>'
    }),
    ...
    ProperName: new FieldDefinition({
        type: 'string',
    ...
    PhoneNumber: new FieldDefinition({
        type: 'string',
        regex: /^[*#\-()\s]{25}$`/,
        gridColDef: {
            minWidth: 120,
...
for (const [key, fd] of Object.entries(BaseFieldDefs)) {
    ...
    if (!FieldDefs[name])
        FieldDefs[name] = from(BaseFieldDefs[key]);
}
export const getFieldDef = (name) => {
    return FieldDefs[name]?.clone() || from(BaseFieldDefs.Default, { name });
};
//# sourceMappingURL=CommonFieldDefinition.js.map