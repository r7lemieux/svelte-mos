...
var errorName_1 = require("./errorName");
...
var Rezult = /** @class */ (function (_super) {
    ...
    function Rezult(errorName, data, context) {
        if (errorName === void 0) { errorName = errorName_1.ErrorName.ok; }
        var _this = _super.call(this) || this;
        _this.status = 'error';
        _this.setName = function (errorName) {
            _this.name = errorName_1.ErrorName[errorName];
        };
        ...
        _this.toDetailString = function () { return (0, string_utils_1.jsonToDisplayString)({
    ...
    Rezult.build = function (err, data, context) {
        if (err && err instanceof Rezult)
            return err;
        var rezult = new Rezult(errorName_1.ErrorName.server_error);
        if (data)
            rezult.data = data;
        if (context)
            rezult.context = context;
        if (err && err.message) {
            rezult.data = __assign(__assign({}, rezult.data), { err: err.message }) || { message: err.message };
        }
        return rezult;
    };
    return Rezult;
...
exports.OK = new Rezult(errorName_1.ErrorName.ok);
